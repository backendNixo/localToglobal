...
export const paymentPlan = async (req, res) => {
  try {
    ...
    if (!plan) {
      return res.status(404).json(new APIError("Plan not found or inactive", 404));
    }
    ...
    return res.status(200).json(
      new APIResponse("Payment initiated successfully !", 200, buyPlan)
    );
...
export const GetTransactionHistory = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json(new APIResponse("Transaction History Fetched Successfully!", 200, history));
  } catch (error) {
    return res
      .status(500)
